...
int CodeBook::learn(IplImage *rawImage){
    IplImage* yuvImage;
    ...
    // 指向yuvImage 图像的通道数据
    for (int i = 0 ; i < imageLen ; i++){
        cvupdateCodeBook(pColor,this->cB[i],cbBounds,nChannels);
...
int CodeBook::Diff(IplImage* rawImage,IplImage* &ImaskCodeBook){
    ...
    uchar *pColor = (uchar *)((yuvImage)->imageData); //3 channel yuv image
    uchar *pMask = (uchar *)((ImaskCodeBook)->imageData);
    //1 channel image
    // 指向ImaskCodeBook 通道数据序列的首元素
    for (int c = 0; c<imageLen; c++)
    {
        uchar maskPixelCodeBook = cvbackgroundDiff(pColor, cB[c], nChannels, minMod, maxMod);
